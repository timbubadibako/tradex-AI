...
import { 
  Activity, BarChart3, History, BrainCircuit, Target, Zap, ChevronDown, CheckCircle2, XCircle, ShieldCheck, Wallet, AlertTriangle
} from "lucide-react";
...
function MarketContent() {
  ...
  // Detect Signal Conflict: AI wants to trade but OBI disagrees
  ...
  useEffect(() => {
    ...
    const sync = async () => {
        if (coin && coin !== status?.coin) await fetch(`http://localhost:8000/api/set_coin?coin=${coin}`, { method: 'POST' });
        if (tf && tf !== status?.tf) await fetch(`http://localhost:8000/api/set_timeframe?tf=${tf}`, { method: 'POST' });
    };
    sync();
  }, [searchParams]);
  ...
  const handleCoinSwitch = async (coin: string) => {
    ...
    await fetch(`http://localhost:8000/api/set_coin?coin=${coin}`, { method: 'POST' });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <GlassCard className="lg:col-span-3 flex flex-col p-10 border-white shadow-xl relative min-h-[700px] bg-white/80" delay={0.1}>
          ...
          <div className="mt-10 grid grid-cols-1 md:grid-cols-3 gap-6">
             <div className="p-7 rounded-[40px] bg-white/60 border border-white shadow-sm flex items-center gap-6">
                ...
                <div>
                   <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1.5">Sniper Projection</p>
                   <p className="text-xl font-black text-slate-900 tracking-tight">Rp {(prediction?.price ?? 0).toLocaleString('id-ID')}</p>
                </div>
             </div>
             <div className="p-7 rounded-[40px] bg-white/60 border border-white shadow-sm flex items-center gap-6">
                ...
                <div>
                   <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1.5">Signal Conviction</p>
                   <p className="text-xl font-black text-slate-900 tracking-tight">{(prediction?.confidence * 100).toFixed(1)}% · {prediction?.signal}</p>
                </div>
             </div>
             <div className="p-7 rounded-[40px] bg-white/60 border border-white shadow-sm flex items-center gap-6">
                <div className="p-4 bg-purple-50 rounded-3xl"><BrainCircuit className="w-8 h-8 text-purple-500" /></div>
                <div>
                   <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1.5">Neural Status</p>
                   <p className="text-sm font-black text-slate-900 uppercase">{(status?.error_rate ?? 0).toFixed(2)}% MAPE · <span className={prediction?.macro === 'BULLISH' ? 'text-emerald-500' : 'text-rose-500'}>{prediction?.macro}</span></p>
                </div>
             </div>
          </div>
        </GlassCard>
        ...
        <div className="flex flex-col gap-6">
           ...
           <GlassCard className="border-white p-8 flex flex-col justify-center min-h-[260px] shadow-lg bg-white/80" delay={0.25}>
              <div className="flex items-center gap-3 mb-6 text-emerald-500"><Activity className="w-5 h-5" /><h3 className="font-black text-xs uppercase tracking-widest text-slate-800">Performance</h3></div>
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-slate-100 pt-8">
                 <button onClick={() => fetch('http://localhost:8000/api/manual_buy', {method:'POST'})} className="py-4 rounded-[20px] bg-emerald-500 text-white font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-emerald-100 active:scale-95 transition-all">Buy</button>
                 <button onClick={() => fetch('http://localhost:8000/api/manual_sell', {method:'POST'})} className="py-4 rounded-[20px] bg-rose-500 text-white font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-rose-100 active:scale-95 transition-all">Sell</button>
              </div>
           </GlassCard>
           ...
           <GlassCard className="bg-slate-50/20 border-white p-8 flex flex-col justify-center min-h-[220px] shadow-sm bg-white/80" delay={0.3}>
              ...
              <div className="space-y-4">
                 ...
                 {isConflict && (
                    <div className="mt-4 p-3 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center gap-3 animate-pulse">
                       <AlertTriangle className="w-5 h-5 text-amber-600" />
                       <span className="text-[9px] font-black text-amber-700 uppercase tracking-tighter">Signal Conflict Detected</span>
                    </div>
                 )}
              </div>
           </GlassCard>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <GlassCard className="lg:col-span-3 flex flex-col border-white p-10 shadow-lg bg-white/80" delay={0.4}>
          <div className="flex items-center justify-between mb-8 sticky top-0 bg-white/10 z-10 backdrop-blur-sm">
            <div className="flex items-center gap-4"><History className="w-7 h-7 text-sky-500" /><h3 className="font-black text-slate-800 text-base uppercase tracking-[0.2em]">Execution Ledger</h3></div>
            <span className="text-xs font-black text-slate-400 uppercase bg-slate-50 px-6 py-2.5 rounded-full border border-slate-100 shadow-sm tracking-widest">Live Data Stream</span>
          </div>
                ...
                {trades.length === 0 ? <tr><td className="py-24 text-center text-slate-300 italic text-base uppercase tracking-[0.3em] animate-pulse">Scanning market signals...</td></tr> : trades.map((t: any, i: number) => (
                  <tr key={i} className="transition-all hover:translate-x-3 group cursor-default">
                    ...
                    <td className="py-5 bg-white border-y border-slate-50">
                      <span className={`px-6 py-2 rounded-2xl text-xs font-black uppercase shadow-lg ${t.type.includes('LONG') ? 'bg-emerald-500 text-white':'bg-rose-500 text-white'}`}>{t.type.replace('_',' ')}</span>
                    </td>
        ...
        <GlassCard className="flex flex-col border-white shadow-lg p-10 bg-white/80" delay={0.5}>
           ...
           <div className="flex-1 flex flex-col justify-center space-y-10 px-2">
              ...
              <div className="text-center leading-none pt-4">
                 ...
                 <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-5">Market Sentiment</p>
              </div>
           </div>
        </GlassCard>
      </div>
...
export default function MarketPage() {
  return (
    <div className="flex min-h-screen bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-sky-50 via-white to-sky-100 font-sans selection:bg-sky-100 overflow-x-hidden">
      <Sidebar />
      <main className="flex-1 flex flex-col p-8 md:p-16 overflow-y-auto no-scrollbar">
